...
export default function RegisterPage() {
  ...
  return (
    <main style={{ minHeight: "calc(100vh - 71px)", display: "grid", placeItems: "center", padding: "clamp(30px,6vw,70px) 20px" }}>
      <div style={{ width: "min(460px,100%)", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 24, boxShadow: "var(--shadow-lg)", padding: "clamp(28px,4vw,40px)", display: "grid", gap: 18 }}>
        <div style={{ textAlign: "center", display: "grid", gap: 8, justifyItems: "center" }}>
          ...
          <h1 style={{ fontFamily: "var(--font-head)", fontWeight: 500, fontSize: "1.9rem", margin: "6px 0 0", letterSpacing: "-0.02em" }}>Crie sua conta</h1>
          <p style={{ margin: 0, color: "var(--muted)", fontSize: ".96rem" }}>Gratuito, para sempre. Comece em menos de um minuto.</p>
        </div>
        <form onSubmit={handleRegister} style={{ display: "grid", gap: 18 }}>
          <div style={{ display: "grid", gap: 6 }}>
            <label style={{ fontSize: ".88rem", fontWeight: 600, color: "var(--muted)" }}>Nome completo</label>
            <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Seu nome" style={{ width: "100%", height: 50, border: "1px solid var(--border)", borderRadius: 13, background: "var(--surface-2)", color: "var(--text)", padding: "0 15px", fontSize: "1rem" }} />
          </div>
          <div style={{ display: "grid", gap: 6 }}>
          ...
          <div style={{ display: "grid", gap: 6 }}>
            ...
            <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Crie uma senha segura" minLength={6} style={{ width: "100%", height: 50, border: "1px solid var(--border)", borderRadius: 13, background: "var(--surface-2)", color: "var(--text)", padding: "0 15px", fontSize: "1rem" }} />
          </div>
        ...
        <p style={{ margin: 0, textAlign: "center", color: "var(--muted)", fontSize: ".95rem" }}>Já tem conta? <Link href="/login" style={{ color: "var(--primary)", fontWeight: 700 }}>Entrar</Link></p>
      </div>
    </main>
  );
}
